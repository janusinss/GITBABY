...
async function loadProfile() {
    ...
    if (result.success && result.data) {
        ...
        document.querySelector('.hero-text').textContent = profile.hero_bio || profile.bio;
...
async function loadManageTab(tabName) {
    ...
    // --- HOBBIES TAB ---
        ...
        const result = await apiCall('hobbies_api.php', 'read', { params: `&profile_id=${PROFILE_ID}` });
        ...
        if (result.success && result.data) {
            result.data.forEach(h => {
                html += `<tr>
                    <td><div class="flex-align"><img src="${h.icon || 'img/default_icon.png'}" class="mini-icon"> ${h.name}</div></td>
                    <td><span class="tag-badge">${h.category}</span></td>
                    <td>
                        <button class="action-btn btn-edit" onclick="openForm('hobbies', ${h.id})">Edit</button>
                        <button class="action-btn btn-delete" onclick="deleteItem('hobbies', ${h.id})">Delete</button>
                    </td>
                </tr>`;
            });
        }
            ...
            html = `
                <div class="profile-view-card">
                    ...
                    <div class="profile-full-width">
                        <label>Mini Profile (Hero)</label>
                        <p class="bio-text">${p.hero_bio || 'No hero bio set.'}</p>
                    </div>
...
function generateFormHTML(type, data, action) {
        ...
        <div class="form-group">
            ...
            <div class="file-upload-container">
                <div class="image-preview" id="preview-${name}">
                    ${value ? `<img src="${value}" alt="Preview">` : '<span style="color:#9ca3af">No image selected</span>'}
                </div>
    ...
    // --- HOBBIES ---
        ...
        fields = `
        <div class="form-group"><label>Name</label><input type="text" name="name" value="${val('name')}" required></div>
        <div class="form-group"><label>Description</label><textarea name="description" rows="2">${val('description')}</textarea></div>
    ...
    // --- PROFILE ---
        ...
        <div class="form-row">
            <div class="form-group"><label>Facebook</label><input type="text" name="facebook" value="${val('facebook')}"></div>
            <div class="form-group"><label>GitHub</label><input type="text" name="github" value="${val('github')}"></div>
        </div>
